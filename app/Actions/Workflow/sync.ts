...
function settingsWith(
  ...
  attributes: { skip?: string | null, softFail?: unknown, branches?: readonly string[] } = {},
): string | null {
  const merged = {
    ...settings,
    ...(environment ? { environment } : {}),
    // Buildkite's step attributes ride here for the same reason the
    // environment does: a run copies its settings forward, and a runner reads
    // what the file asked for rather than a definition it may no longer match.
    ...(attributes.skip ? { skip: attributes.skip } : {}),
    ...(attributes.softFail ? { softFail: attributes.softFail } : {}),
    ...(attributes.branches && attributes.branches.length > 0 ? { branches: attributes.branches } : {}),
...
async function insertVersion(
  ...
  const version = await db
    ...
    .values({
      ...
      on_issues: triggers.issues !== null,
      issue_types: lines(triggers.issues?.types ?? []),
      on_issue_comment: triggers.issueComment !== null,
      issue_comment_types: lines(triggers.issueComment?.types ?? []),
      on_release: triggers.release !== null,
      release_types: lines(triggers.release?.types ?? []),
      on_repository_dispatch: triggers.repositoryDispatch !== null,
      repository_dispatch_types: lines(triggers.repositoryDispatch?.types ?? []),
      on_workflow_run: triggers.workflowRun !== null,
      workflow_run_workflows: lines(triggers.workflowRunWorkflows),
      workflow_run_types: lines(triggers.workflowRun?.types ?? []),
      workflow_run_branches: lines(triggers.workflowRun?.branches ?? []),
