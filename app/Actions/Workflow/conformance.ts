...
export const CONFORMANCE: ConformanceEntry[] = [
  ...
  { key: 'steps[*].timeout-minutes', level: 'step', status: 'supported', behaviour: 'Enforced per step by the runner, which is the narrow and more useful of the two timeouts: a job allowed sixty minutes that hangs in a thirty-second health check spends fifty-nine of them proving nothing. A step stopped this way says so rather than reading as an ordinary failure.' },
  ...
  { key: 'reviewos.wait / block / trigger / group / if-changed / retry / priority / agents / parallelism / artifact-paths / secrets / cancel-on-build-failing / checkout', level: 'job', status: 'differs', behaviour: 'This instance\'s own extensions, and the only keys here that are not Actions\': a barrier, a gate a person opens, a job that starts another run, a label that groups jobs on the screen, per-job path gating for a monorepository, an automatic retry with a required cap, a queue priority, a `key=value` tag query over what machines report about themselves, `parallelism:` - one job definition run N times, each copy told which it is - `artifact-paths:`, globs the runner collects when the job ends whether it passed or failed - and `secrets:`, naming which credentials a job is given, since a job that receives every secret in scope is a job whose compromised dependency reads the deploy key it never needed - `cancel-on-build-failing:`, which stops a long job once something else has already sunk the run - and `checkout:`, which is depth, sparse paths, submodules, LFS or nothing at all, said on the job rather than as a step with four `with:` keys. All four live under one `reviewos:` key so there is one thing to delete, and **GitHub refuses a file that uses them** rather than ignoring them - which is the right failure, since a `block:` that GitHub ignored would be a deployment approval that approves itself. Documented in [extensions](./extensions.md).' },
