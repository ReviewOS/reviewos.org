...
export const CONFORMANCE: ConformanceEntry[] = [
  ...
  { key: 'reviewos.wait / block / trigger / group / if-changed / retry / priority / agents / parallelism / artifact-paths / secrets / cancel-on-build-failing / checkout / concurrency-group', level: 'job', status: 'differs', behaviour: 'This instance\'s own extensions, and the only keys here that are not Actions\': a barrier, a gate a person opens, a job that starts another run, a label that groups jobs on the screen, per-job path gating for a monorepository, an automatic retry with a required cap, a queue priority, a `key=value` tag query over what machines report about themselves, `parallelism:` - one job definition run N times, each copy told which it is - `artifact-paths:`, globs the runner collects when the job ends whether it passed or failed - and `secrets:`, naming which credentials a job is given, since a job that receives every secret in scope is a job whose compromised dependency reads the deploy key it never needed - `cancel-on-build-failing:`, which stops a long job once something else has already sunk the run - and `checkout:`, which is depth, sparse paths, submodules, LFS or nothing at all, said on the job rather than as a step with four `with:` keys - and `concurrency-group:` with `concurrency:` and `concurrency-method:`, a named limit shared by every job wearing it across every run and workflow in the repository, which is the deploy lock a run-level group cannot express. All four live under one `reviewos:` key so there is one thing to delete, and **GitHub refuses a file that uses them** rather than ignoring them - which is the right failure, since a `block:` that GitHub ignored would be a deployment approval that approves itself. Documented in [extensions](./extensions.md).' },
