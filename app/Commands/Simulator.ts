...
// open longest, and it will be asked again after every change to the viewer.
//
...
// missing, because the failures are all environmental and none of them say what
// to do: `xcode-select` pointing at the command line tools reports a missing
// `xcodebuild`, and a machine with no runtime installed reports an empty device
// list rather than "download a runtime". `simulator:open` is the thing worth
// repeating: boot a phone, open a URL in Mobile Safari, and optionally take a
// picture of what it did.
//
...
// `\/\/`, and pickier reads those two characters as the start of a line
// comment - so every use of a variable after it in the same function
// disappears, and the rule that fires is an unused-parameter error that points
// nowhere near the cause. Parsing is also the better check: it rejects the
// shapes a pattern lets through.
...
export default function (cli: CLI) {
  ...
  cli
    ...
    .action(async (url: string, options: any) => {
      ...
      //
      // Line comments rather than a block, throughout this file: a block comment
      // inside a function body makes pickier's unused-variable rule flag every
